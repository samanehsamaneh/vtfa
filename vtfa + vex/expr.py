...
class GetI(IRExpr):
   ...
   def getreg(expr):
    print "GETREG(%s) type %s" % (expr.offset, expr.ty)

  def getarg(arg):
    if isinstance(arg, pyvex.IRExpr.RdTmp):
        return "TMP(%d)" % (arg.tmp)
    elif isinstance(arg, pyvex.IRExpr.Const):
        return "CONST"
    else:
        print "UNKNOWN ARG type %s" % (type(arg).__name__)
        exit(-1)
...
class Binop(IRExpr):
    ...
    def binop(expr):
        print "BINOP %s(%s,%s)" % (expr.op, getarg(expr.args[0]), getarg(expr.args[1]))
